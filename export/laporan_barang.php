...
<?php
	...
	if($dataAct=='PDF'){
		$report = $c->reportService()->runReport('/reports/Electrical/Laporan_Barang', 'pdf', null, null, $control);
        ...
        header('Content-Disposition: attachment; filename="Laporan_Barang_'.date('ymdhis').'.pdf"');
        echo $report;
	}elseif($dataAct=='EXCEL'){
		$report = $c->reportService()->runReport('/reports/Electrical/Laporan_Barang', 'xls', null, null, $control);
        header('Cache-Control: must-revalidate');
        header('Content-Transfer-Encoding: binary');
        header('Content-Length: ' . strlen($report));
