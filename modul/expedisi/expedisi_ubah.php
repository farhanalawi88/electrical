<?php
	if(isset($_POST['btnSave'])){
		...
		if (trim($_POST['txtNama'])=="") {
			$message[] = "<b>Nama expedisi</b> tidak boleh kosong!";		
		}
		...
		if(count($message)==0){	
			$qryUpdate=mysql_query("UPDATE ms_expedisi SET nama_expedisi='$txtNama', 
								  							keterangan_expedisi='$txtKeterangan',
															status_expedisi='$cmbStatus'
													WHERE kode_expedisi ='".$_POST['txtKode']."'") 
					   or die ("Gagal query update".mysql_error());
			if($qryUpdate){
				$_SESSION['info'] = 'success';
				$_SESSION['pesan'] = 'Data expedisi berhasil diperbaharui';
	...
	$sqlShow = "SELECT * FROM ms_expedisi WHERE kode_expedisi='".decrypt($KodeEdit)."'";
	$qryShow = mysql_query($sqlShow, $koneksidb)  or die ("Query ambil data expedisi salah : ".mysql_error());
	$dataShow = mysql_fetch_array($qryShow);
	
	$dataKode		= $dataShow['kode_expedisi'];
	$dataNama		= isset($dataShow['nama_expedisi']) ?  $dataShow['nama_expedisi'] : $_POST['txtNama'];
	$dataKeterangan	= isset($dataShow['alamat_expedisi']) ?  $dataShow['alamat_expedisi'] : $_POST['txtKeterangan'];
	$dataStatus 	= isset($dataShow['status_expedisi']) ?  $dataShow['status_expedisi'] : $_POST['cmbStatus'];
?>
		
<div class="portlet box green">
	<div class="portlet-title">
		<div class="caption"><span class="caption-subject uppercase">Form Perubahan Expedisi</span></div>
		<div class="tools">
	...
	<div class="portlet-body form">
		<form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" name="frmadd" class="form-horizontal form-bordered">
			<div class="form-body">
				...
				<div class="form-group">
					<label class="col-md-2 control-label">Nama Expedisi :</label>
					<div class="col-md-5">
			    ...
			    <div class="row">
			        <div class="form-group">
			            <div class="col-lg-offset-2 col-lg-10">
			                ...
			                <a href="?page=dtexpds" class="btn green"><i class="fa fa-undo"></i> Batalkan</a>
			            </div>
			        </div>
			    </div>
			</div>
