...
function kodeUnik($tabel, $field, $inisial, $panjang,$tanggal){
 	...
 	$qry	= mysql_query("SELECT MAX(".substr($field,0,3).") FROM $tabel WHERE YEAR(".$tanggal.")='".date('Y')."' AND MONTH(".$tanggal.")='".date('m')."'");
 	...
 	$angka	=strval($angka); 
 	$tmp	="";
 	for($i=1; $i<=(strlen($inisial)-$panjang-strlen($angka)); $i++) {
		$tmp=$tmp."0";	
	}
...
function kodeBarang($inisial, $panjang,$kategori){

 	$qry	= mysql_query("SELECT kode_barcode FROM ms_barang WHERE kode_kategori LIKE ".$kategori."");
 	$row	= mysql_fetch_array($qry); 
 	if ($row[0]=="") {
 		$angka=0;
	}
